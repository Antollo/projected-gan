    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg, fp32=fp32)
        ...
        if cur_tick * kimg_per_tick <= 500:
            early_snapshots = cur_tick % (image_snapshot_ticks // 2) == 0
        ...
        if (rank == 0) and (restart_every > 0) and (network_snapshot_ticks is not None) and (
            ...
            with open(snapshot_pkl.replace('.pkl', f'-{cur_nimg//1000:06d}.pkl'), 'wb') as f:
                pickle.dump(snapshot_data, f) 
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], run_dir=run_dir, cur_nimg=cur_nimg,
                    dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, batch_gpu=batch_gpu)
                if rank == 0:
